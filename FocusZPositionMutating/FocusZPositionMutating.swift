...
extension UIView {
    ...
    public static func fzpm_swizzleDidUpdateFocus(focusedZPosition: CGFloat = 1.0,
                                                  unfocusedZPosition: CGFloat = 0.0) throws {
        ...
        do {
            let method: Method = class_getInstanceMethod(object_getClass(instance),
                                                         #selector(didUpdateFocus(in:with:)))!

            let swizzledMethod: Method = class_getInstanceMethod(object_getClass(instance),
                                                                 #selector(fzpm_didUpdateFocus(in:with:)))!
        ...
        do {
            ...
            let swizzledMethod: Method = class_getInstanceMethod(object_getClass(instance),
                                                                 #selector(fzpm_didMoveToSuperview))!
    ...
    @objc
    func fzpm_didUpdateFocus(in context: UIFocusUpdateContext,
...
extension FocusZPositionMutating where Self: UIView {
    ...
    public func applyCoordinatedZPositionState(context: UIFocusUpdateContext,
        ...
        if let next = context.nextFocusedView, next.isDescendant(of: self) {
            ...
            log()
        } else if let previous = context.previouslyFocusedView,
